  ...
  if (loopBackModels) {
    for (var modelIdx in loopBackModels) {
      ...
      for (var propertyName in loopBackModel.definition.rawProperties) {
        ...
        var refType = undefined;
        if (!listType && DATA_TYPES.indexOf(loopBackProperty.type) == -1) {
          refType = loopBackProperty.type;
        }
        var items = listType ? { $ref: listType } : loopBackProperty.items;
        swaggerModel.properties[propertyName] = {
          description: loopBackProperty.doc,
          required: loopBackProperty.required,
          type: listType ? 'array' : (refType ? undefined : loopBackProperty.type),
          $ref: refType,
          items: items,
