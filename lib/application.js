...
app.isListType = function (dataType) {
  if (typeof dataType == 'string' && dataType.indexOf('[') >= 0) {
...
DATA_TYPES = ['array', 'boolean', 'byte', 'date', 'dateTime', 'double', 'File', 'float',
  'integer', 'list', 'long', 'null', 'number', 'object', 'set', 'string', 'void'];
app.convertToSwagger = function (loopBackModels) {
  var swaggerModels = {};
  if (loopBackModels) {
    for (var modelIdx in loopBackModels) {
      var loopBackModel = loopBackModels[modelIdx];
      var modelName = loopBackModel.definition.name;
      swaggerModel = {
        id: modelName,
        discriminator: loopBackModel.definition.discriminator,
        description: loopBackModel.definition.description,
        properties: {},
        required: [],
        subTypes: loopBackModel.definition.subTypes
      };
      for (var propertyName in loopBackModel.definition.rawProperties) {
        loopBackProperty = loopBackModel.definition.rawProperties[propertyName];
        if (loopBackProperty.required) {
          swaggerModel.required.push(propertyName);
        }
        var listType = app.isListType(loopBackProperty.type);
        swaggerModel.properties[propertyName] = {
          description: loopBackProperty.doc,
          required: loopBackProperty.required,
          type: listType ? 'array' : loopBackProperty.type,
          $ref: loopBackProperty.type,
          items: loopBackProperty.items,
          format: loopBackProperty.format,
          defaultValue: loopBackProperty.defaultValue,
          enum: loopBackProperty.enum,
          minimum: loopBackProperty.minimum,
          maximum: loopBackProperty.maximum,
          uniqueItems: loopBackProperty.uniqueItems,
          allowMultiple: loopBackProperty.allowMultiple,
          description: loopBackProperty.description
        };
        if (listType || DATA_TYPES.indexOf(loopBackProperty.type)) {
          swaggerModel.properties[propertyName].type = listType ? 'array' : loopBackProperty.type;
        } else {
          swaggerModel.properties[propertyName].$ref = loopBackProperty.type;
        }
        if (listType) {
          swaggerModel.properties[propertyName].items = {
            $ref: listType
          };
        }
      }
      swaggerModels[modelName] = swaggerModel;
    }
  }
...
app.docs = function (options) {
  ...
  var swaggerModels = app.convertToSwagger(this.models());
  swagger(remotes, options, swaggerModels);
}
